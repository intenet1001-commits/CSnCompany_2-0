---
description: "CS 세션 종료 자동화 - Session Digest → 4-Agent 병렬 분석 → 학습 게이트 → Selective 버전업 → GitHub push → (선택) 워크트리 머지 → 구조화 compact 제안 (/cs-end)"
...
**v4 신규:**
- **워크트리 머지** (Phase 4.5, `--merge-worktree`) — `/cs-start`(cs-worktree)로 만든 활성 워크트리를 찾아 base 브랜치로 반영. 실행 시 PR 생성(기본) vs 직접 머지를 물어보고, conflict 감지 시 자동 해결 없이 중단. `--validate` 로 머지 전 cs-ship 검증을 선택적으로 켤 수 있다. **명시적 옵트인** — 플래그 없으면 이 Phase는 조용히 스킵되며, cs-end의 기존 "마켓플레이스 레포만 자동 커밋, 프로젝트 디렉토리는 손대지 않는다" 경계는 그대로 유지된다.
...
4.5. **Phase 4.5 — 워크트리 머지** ← v4 신규 (opt-in: `--merge-worktree`, conflict 시 중단)
5. **Phase 5 — Push 완료 리포트** (두 레포 상태 명확 구분 출력 + 워크트리 머지 결과)
...
**v4 플래그 (원시 인자에서 직접 파싱 — prepass가 모를 수 있으므로):**

```bash
MERGE_WORKTREE=false      # --merge-worktree: Phase 4.5 활성화
VALIDATE_BEFORE_MERGE=false  # --validate: 머지 전 cs-ship 검증
for arg in "$@"; do
  case "$arg" in
    --merge-worktree) MERGE_WORKTREE=true ;;
    --validate)       VALIDATE_BEFORE_MERGE=true ;;
  esac
done
```
...
## Phase 4.5 — 워크트리 머지 (opt-in, v4 신규)

**Skip guard (이 Phase의 첫 판정):** `MERGE_WORKTREE != true` 이면 이 Phase 전체를 **조용히 스킵**한다 (출력 없음). 플래그가 없으면 v3와 동일하게 동작하며, 아래 로직은 실행되지 않는다.

> **경계 유지:** 이 Phase는 프로젝트 저장소의 미커밋 변경을 자동 커밋하지 않는다. `/cs-start`로 만든 **워크트리 브랜치**(이미 커밋된 작업)를 base 브랜치로 반영할 뿐이다. Phase 4의 "프로젝트 디렉토리는 손대지 않는다" 원칙과 충돌하지 않는다 — 사용자가 명시적으로 `--merge-worktree`를 켰을 때만, 명시적으로 물어본 방식으로만 동작한다.

### 1. 대상 워크트리 탐색 (cs-worktree 매니페스트)

```bash
STATE_DIR="$HOME/.claude/state/cs-worktree"
MANIFEST="$STATE_DIR/manifest.json"
if [ ! -f "$MANIFEST" ]; then
  echo "ℹ️  cs-worktree 매니페스트가 없습니다 ($MANIFEST). 머지할 워크트리 없음 — Phase 4.5 스킵."
  # → Phase 5로 진행 (에러 아님)
fi
```

대상 프로젝트(`$MERGE_TARGET`)를 확정한다 — `EXPLICIT_PROJECT`(있으면) 또는 `PROJECT_DIR`. git toplevel로 정규화한 뒤, 매니페스트에서 `project`가 일치하고 `status == "active"`인 항목을 조회한다:

```bash
MERGE_TARGET=$(git -C "${EXPLICIT_PROJECT:-$PROJECT_DIR}" rev-parse --show-toplevel 2>/dev/null)
ACTIVE=$(python3 - "$MANIFEST" "$MERGE_TARGET" <<'PY'
import json, sys
manifest, target = sys.argv[1], sys.argv[2]
d = json.load(open(manifest))
rows = [w for w in d.get("worktrees", []) if w.get("project") == target and w.get("status") == "active"]
print(json.dumps(rows, ensure_ascii=False))
PY
)
```

- **0개** → "이 프로젝트에 활성 cs-worktree 없음" 한 줄 출력 후 Phase 5로 진행 (에러 아님).
- **1개** → 그 항목 사용.
- **2개 이상** → AskUserQuestion으로 어떤 워크트리를 머지할지 선택하게 한다 (id/branch/created_at 표시).

선택된 항목에서 `BRANCH`, `WT_PATH`, `BASE`(base_branch)를 추출한다.

### 2. 머지 전 사전 점검 (비파괴)

다음을 순서대로 확인하고, 하나라도 실패하면 **머지하지 않고 중단 + 안내**한다:

```bash
# (a) 워크트리 워킹트리가 clean 한가 (미커밋 변경이 있으면 사용자가 먼저 커밋해야 함 — 자동 커밋 금지)
git -C "$WT_PATH" diff --quiet && git -C "$WT_PATH" diff --cached --quiet || {
  echo "⚠️ 워크트리에 미커밋 변경이 있습니다: $WT_PATH"
  echo "   먼저 커밋한 뒤 다시 /cs-end --merge-worktree 를 실행하세요 (cs-end는 프로젝트 변경을 자동 커밋하지 않습니다)."
  # → 중단, Phase 5에 SKIPPED(dirty) 보고
}

# (b) base 대비 실제 머지할 커밋이 있는가
AHEAD=$(git -C "$MERGE_TARGET" rev-list --count "$BASE..$BRANCH" 2>/dev/null)
[ "${AHEAD:-0}" -gt 0 ] || {
  echo "ℹ️ '$BRANCH' 에 '$BASE' 대비 새 커밋이 없습니다. 머지 생략."
  # → 중단, Phase 5에 SKIPPED(no-commits) 보고
}
```

### 3. (선택) cs-ship 검증 — `--validate`

`VALIDATE_BEFORE_MERGE == true` 이면 머지 전에 cs-ship 을 실행한다:

```bash
LATEST_SHIP=$(ls -d "$MARKETPLACE_DIR/plugins/cs-ship-v"* 2>/dev/null | sort -V | tail -1)
```

`cs-ship`의 `ship-lead` 에이전트를 Task()로 스폰해 워크트리(`$WT_PATH`)를 대상으로 pre-ship 검증을 수행한다. 판정이 **PASS가 아니면** 머지하지 않고 중단하며, ship-lead의 MISSING/PARTIAL 요약을 출력한다. (`--validate`가 없으면 이 단계는 건너뛴다.)

### 4. 머지 방식 선택 (PR 기본)

AskUserQuestion으로 머지 방식을 묻는다 — 의도: 이 워크트리 브랜치를 base에 어떻게 반영할지. **기본값은 PR 생성**이다. 옵션:

- **PR 생성 (기본)** — `gh pr create` 로 PR을 만들고 사용자가 GitHub에서 리뷰/머지.
- **직접 머지** — base 브랜치로 로컬 머지 (conflict 시 중단).
- **이번엔 스킵** — 머지하지 않고 워크트리를 active로 유지.

문구는 자유. 사용자가 응답하지 않거나 비대화 환경이면 **PR 생성을 기본**으로 택한다.

### 5-A. PR 생성 경로 (기본)

```bash
# 워크트리 브랜치를 origin에 push (upstream 설정)
git -C "$WT_PATH" push -u origin "$BRANCH"
# gh 로 PR 생성 (기본 draft 아님 — 필요 시 --draft 안내)
gh --version >/dev/null 2>&1 && \
  git -C "$WT_PATH" ... # gh pr create --base "$BASE" --head "$BRANCH" --fill (레포 컨텍스트에서 실행)
```

- `gh` 미설치 또는 인증 실패 → PR을 만들지 못했음을 알리고, 수동 명령(`gh pr create --base $BASE --head $BRANCH --fill`)을 출력한다. 브랜치 push 는 유지한다.
- PR 생성 성공 → 매니페스트 항목 `status`를 `pr-open`으로 갱신하고 PR URL을 기록한다.

### 5-B. 직접 머지 경로

**conflict를 자동 해결하지 않는다.** 비파괴 사전 점검 후 클린할 때만 머지한다:

```bash
# git 2.38+ 이면 비-뮤테이팅 사전 점검 (merge-tree)
if git -C "$MERGE_TARGET" merge-tree --write-tree "$BASE" "$BRANCH" >/tmp/cs_mt.$$ 2>/dev/null; then
  CONFLICTS=$(grep -c "CONFLICT" /tmp/cs_mt.$$ 2>/dev/null || echo 0)
else
  CONFLICTS=unknown
fi
```

- **conflict 감지 (또는 merge-tree 미지원 시 실제 머지 시도 중 conflict 발생)** → **자동 해결 없이 중단**한다. `git merge --abort`로 원상 복구하고, 수동 해결 안내를 출력한다:
  ```
  ⚠️ '$BRANCH' → '$BASE' 머지 중 충돌이 감지되었습니다. 자동 해결하지 않습니다.
     수동 해결: cd $MERGE_TARGET && git checkout $BASE && git merge $BRANCH
     충돌 해결 후 커밋하거나, PR 방식(gh pr create)으로 전환하세요.
  ```
  워크트리는 active로 유지하고 Phase 5에 `HALTED(conflict)`로 보고한다.
- **conflict 없음** → base 브랜치에서 머지를 수행한다. base가 다른 워크트리/메인에 체크아웃되어 있을 수 있으므로, 안전하게 base를 대상으로 `--no-ff` 머지를 진행하고 결과를 확인한다. 성공 시 매니페스트 `status`를 `merged`로 갱신한다.

> **force-push·브랜치 삭제 금지:** 어느 경로든 워크트리/브랜치를 자동 삭제하지 않는다. 정리(worktree remove / branch -d)는 사용자가 명시적으로 수행하도록 안내만 한다.

### 6. 매니페스트 상태 갱신

머지/PR 결과에 따라 해당 항목의 `status`를 `merged` | `pr-open` | (스킵/중단 시) `active` 유지로 갱신한다 (inline python3). Phase 5 리포트에 결과를 전달한다.
...
- **워크트리 머지** (Phase 4.5가 실행된 경우에만): 대상 브랜치 + 결과 상태 — `MERGED`(base로 머지됨) / `PR-OPEN`(PR URL) / `HALTED(conflict)`(수동 해결 안내) / `SKIPPED(<사유>)`(no-commits, dirty, no-active-worktree, validate-fail 등). Phase 4.5가 스킵된 경우(플래그 없음) 이 줄은 출력하지 않는다.
...
/cs-end --merge-worktree                       # (v4) /cs-start로 만든 활성 워크트리를 base로 머지 (PR 기본, conflict 시 중단)
/cs-end --merge-worktree --validate            # (v4) 머지 전 cs-ship 검증을 먼저 통과해야 진행
/cs-end --project /path/to/proj --merge-worktree  # (v4) 대상 프로젝트 명시 + 워크트리 머지
```

## v4 워크트리 워크플로우 (cs-worktree 연동)

```
/cs-start /path/to/project        # 격리 워크트리 + cs-work/<slug> 브랜치 생성, 매니페스트 기록
cd <worktree_path>                # 그 안에서 작업 + 커밋
/cs-end --merge-worktree          # 작업 종료 시 워크트리를 base로 반영 (PR 기본 / 직접 머지 선택)
```

- 머지 방식은 실행 시 물어보며 **기본값은 PR**(`gh pr create`).
- conflict 발생 시 자동 해결하지 않고 중단 + 수동 해결 안내.
- `--validate`로 머지 전 cs-ship 검증을 켤 수 있다.
- 이 기능은 **명시적 옵트인** — `--merge-worktree`가 없으면 cs-end는 v3와 동일하게 동작한다.
