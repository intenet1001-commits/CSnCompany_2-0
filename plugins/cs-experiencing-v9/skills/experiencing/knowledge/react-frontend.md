...
### 140. 표시 이름과 권한 플래그가 분리된 인증 구조에서는 이름 충돌이 사일런트 권한 오판을 만든다 (2026-07-17)
<!-- tier: principle -->

- **상황**: 비밀번호 기반 관리자 pseudo-login이 화면에 고정 표시이름("관리자")을 보여주는 앱에서, 별도의 일반 이름-로그인 경로도 동일한 문자열을 실제 회원 이름으로 등록 가능했다.
- **발견**: 사용자가 일반 이름 로그인 폼에 "관리자"를 직접 입력하면 그 이름의 기존 회원 계정으로 로그인되어 화면엔 진짜 관리자와 동일하게 "관리자"가 표시되지만 실제 권한 플래그(isAdmin)는 false였다. 사용자는 "관리자로 로그인했는데 관리자 기능이 안 보인다"고 신고했고, 캐시/코드 결함으로 먼저 의심했으나 정확한 재현 시나리오(이름칸에 "관리자" 직접 입력)를 Playwright로 재현해서야 원인이 확정됐다.
- **교훈**: 표시 이름과 실제 권한/역할 플래그가 서로 다른 소스(하나는 문자열 매칭, 하나는 별도 플래그)에서 오는 인증 구조에서는, 권한 있는 계정의 표시 이름(예: "관리자", "admin")을 일반 회원가입/이름-로그인 경로에서 예약어로 차단해야 한다. 권한 관련 버그 신고를 받으면 캐시/배포 문제를 의심하기 전에, 사용자가 실제로 밟았을 가능성이 있는 정확한 입력 시나리오부터 자동화로 재현해 상태값을 직접 확인한다.
- **근거**: Playwright 재현 결과 — "STATE AFTER TYPING 관리자 AS REGULAR NAME: {isAdmin:null, userId:'6c22d9c0-...', userName:'관리자'}" (skeptic verifier CONFIRM — 상태 덤프가 주장을 정확히 재현하며, 표시이름/권한 플래그 분리 구조는 특정 앱 버전에 종속되지 않는 일반 아키텍처 패턴으로 판정).

### 169. 파생·복원 콘텐츠는 출처 상태를 타입 필드로 보존하고 최종 UI까지 노출해 원문 오인을 막는다 (2026-07-26)
<!-- tier: principle -->
- **상황**: 원문에서 파생하거나 손실 후 복원한 콘텐츠(요약·재생성·OCR 복구 등)를 원문과 같은 필드에 담아 화면에 렌더링했다.
- **발견**: 출처 구분이 타입에 없으면 파이프라인 하류로 갈수록 파생물이 원문과 구별 불가능해지고, 최종 사용자는 재생성된 텍스트를 원문으로 신뢰한다. 주석이나 변수명 관례로는 유지되지 않는다.
