...
### 178. 없는 커버리지는 없다고 보고한다 — 미룬 작업은 구조화된 revisit 트리거와 함께 남긴다 (2026-08-22)
<!-- tier: principle -->
- **상황**: 한 번에 완결되지 않는 시스템에서, 의식적으로 남긴 작업과 확신이 과했던 결론이 같은 날 두 건의 위반으로 측정됐다.
- **발견**: 너무 확신 있게 기록한 근본 원인("마이그레이션이 컬럼을 추가하지 않았다")이 실제 한 줄짜리 수정을 인시던트 내내 가렸다. 또 메모리 행 11개를 일괄로 접겠다는 계획은 이미 접힌 7개를, 그것도 이미 초과 크기로 표시된 파일에 중복 생성할 뻔했다. 두 경우 모두 "커버했다"는 암묵적 주장이 실제 상태와 달랐다.
- **교훈**: 존재하지 않는 커버리지를 주장하는 것이 가장 비싼 실수다. 의식적으로 미룬 작업은 침묵으로 두지 말고 (a) 시도하지 않은 것, (b) 미루는 것이 옳았던 이유, (c) 무언가가 평가할 수 있는 구조화된 `revisit` 트리거(`{due, trigger, condition}`)를 적은 deferred 항목으로 남긴다. 검증 스크립트가 `revisits_due`와 `deferred_work`를 매 실행마다 표면화하면 미룬 일이 잊힌 일로 부패하지 않는다.
- **근거**: memory entry `272ca21c124ba4f0918f132f@412a987d85885d70bbe68a764396f937` (nhdesign-marketplace); 같은 원칙을 LOOP-PROTOCOL `[d-2]`에 반영
<!-- provenance: candidate=btw-memory-234f7258ef8a6a92baadbd40; memory=1eb621cd-79c2-46fa-bf38-dd6c2a9a9657 -->
