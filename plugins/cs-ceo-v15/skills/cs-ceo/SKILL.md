...
- **교훈**: 의심되면 호출이 기본값. 동일 세션 내 동일 주제·동일 버전 범위 재호출 금지 (발췌가 현재 질문의 API/버전을 직접 다루지 않으면 좁힌 키워드로 재호출). 미설치 시 AskUserQuestion으로 Install/Skip/Abort 3지선다 제시.
...
### 18. CS_V7 위키 연동 제거 — cs-ceo 자체 완결 원칙 (2026-05-30)
- **상황**: Phase -3.5(CS_V7 개인 위키 조회) + cs-end Phase 2.1(CS_V7/raw/ 역류입)이 외부 시스템 의존을 만들었음. cs-ceo가 CS_V7 볼트 없이 단독 실행되지 않는 문제.
