...
- 2026-08-22: Second `/cs-memory:learn` → `/cs-memory:upgrade` cycle. The scheduled collector (launchd, 6h, registry scope) had already queued the backlog, so `learn` only consumed one bounded batch of 5 and `upgrade` drained 12 accumulated candidates: 9 promoted, 2 rejected as project-local, 1 held pending. Three of the promotions were **deferred shared-contract patches from 2026-07-16/17** whose knowledge entries (#118/#119/#122) had long been in `cs-experiencing` while the contract edit itself waited on author review — they became LOOP-PROTOCOL `[d-1]`, `[g] FAN-OUT BRIEFING`, and `[h] PARALLEL WRITE ISOLATION`. `plugins/shared/` is unversioned and outside the marketplace, so contract edits there bump nothing; only `cs-experiencing` 9.1.1 → 9.1.2 and `CS-codebase-review` 29.0.2 → 29.0.3 moved. Retargeting note: #119's deferred target was `agents/verifier.md`, but the rule governs how leads brief workers, so it landed in LOOP-PROTOCOL instead of the refuter agent.
...
- Two-Mac topology (2026-08-22): the learning **inputs** are machine-local and outside git — the collector state (`~/.csncompany/state/memory-learning.json`) and the candidate queue (`~/.claude/.experiencing-btw.json`) both live in `$HOME`. Only the **outputs** (owner skills, experiencing knowledge, versions) are in the repo. So `/cs-memory:learn` and `/cs-memory:upgrade` can be run on one machine and the other only needs `git pull` + restart — but only for projects whose `.agent-memory` reaches the learning machine, i.e. the project is checked out there and its memory is synced. A project that exists only on the other Mac never reaches this collector.
...
- A session can load an **older cached plugin version** than the repo, and the symptoms do not look like a version problem. On 2026-08-22 the session resolved `cs-memory` **2.0.1** from `~/.claude/plugins/cache/CSnCompany_2-0/cs-memory/` while the repo was at 2.1.0: the `schedule` skill was simply absent from the available-skills list (2.0.1 has no such skill), and `/cs-memory:learn` loaded 2.0.1's `long_term_memory_training.py`, which uses a different cursor and would have ignored the 63 pending entry versions the 2.1.0 collector had queued. Root cause: the cache keeps multiple versions side by side and the session pins one. Workaround: read the base directory printed when a skill loads, compare it against the plugin's `VERSION`, and follow the repo version's procedure if they differ — then restart to re-resolve the cache.
